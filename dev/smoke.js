...
/*
 * The hub's demo. Its harness hands a manifest default-value over as the XML
 * string, so an unset TwoOptions "false" arrives true: every preset sets every
 * input. And its stand-in Dataverse searches the tag table's primary name, so
 * the column the fixture binds as the label has to be that column.
 */
{
    const hub = JSON.parse(fs.readFileSync(path.join(root, 'pcfhub.json'), 'utf8'));
    const demo = JSON.parse(fs.readFileSync(path.join(root, hub.demo.datasetFixture), 'utf8'));
    ...
    const missing = hub.demo.presets.map((entry) => [entry.slug, inputs.filter((name) => !Object.hasOwn(entry.props, name))]).filter(([, names]) => names.length > 0);
    const label = demo.columns.find((column) => column.alias === 'labelField');

    check('every hub preset sets every input', inputs.length > 0 && missing.length === 0, JSON.stringify(missing));
    check(
        "the demo's label column is the tag table's primary name, which its search runs on",
        label && demo.dataverse && demo.dataverse.tables[demo.targetEntityType].primaryName === label.name,
    );
}
